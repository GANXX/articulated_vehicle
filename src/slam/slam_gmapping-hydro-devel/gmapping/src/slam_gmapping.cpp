...
SlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
  ...
  gmap_pose = GMapping::OrientedPoint(odom_pose.getOrigin().x(),
                                      ...
                                      yaw);
  odompose1<<gmap_pose.x<<" "<<gmap_pose.y<<" "<<gmap_pose.theta<<endl;
  odompose1.close();
  return true;
}
...
SlamGMapping::getOdomPose(GMapping::OrientedPoint& gmap_pose, const ros::Time& t)
{
  ...
  // Get the laser's pose that is centered
  vector<double> value;
  ifstream file_open("1.txt");
  double value_tmp;
  while(file_open>>value_tmp)
  {
  ...
  x_=value[value.size()-3];
  y_=value[value.size()-2];
  yaw_=value[value.size()-1];
  
  c=cos(yaw_);
  s=sin(yaw_);

  ofstream odompose2;
  odompose2.open("2.txt",ios::app);
  ...
  gmap_pose = GMapping::OrientedPoint(c*odom_pose.getOrigin().x()-s* odom_pose.getOrigin().y()+x_,
                                      s*odom_pose.getOrigin().x()+c* odom_pose.getOrigin().y()+y_,
                                      yaw+yaw_);
  
  odompose2<<gmap_pose.x<<" "<<gmap_pose.y<<" "<<gmap_pose.theta<<endl;
  odompose2.close();
